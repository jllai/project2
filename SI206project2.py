...
def grab_headlines():
    inp = 'http://www.michigandaily.com/section/opinion'
    txt_file = requests.get(inp)
    soup = BeautifulSoup(txt_file.text, "lxml")
    opinions = []
    # all_lists = soup.find_all('li')
    for most_read in soup.find_all(class_='view view-most-read view-id-most_read view-display-id-panel_pane_1 view-dom-id-99658157999dd0ac5aa62c2b284dd266'):
        return most_read.text.split('\n')
...
def get_umsi_data():
    ...
    while i < count:
        ...
        for names in soup.find_all(class_="field field-name-title field-type-ds field-label-hidden"):
            names_lst.append(names.h2.text)

        for titles in soup.find_all(class_="field field-name-field-person-titles field-type-text field-label-hidden"):
            title_lst.append(titles.text)

        if base_url != 'https://www.si.umich.edu/directory?field_person_firstname_value=&field_person_lastname_value=&rid=All&page=12':
            for next_page in soup.find(class_="pager-next last"):
                base_url = 'https://www.si.umich.edu' + next_page.get('href')

        i += 1
    
    umsi_titles = dict(zip(names_lst, title_lst))
